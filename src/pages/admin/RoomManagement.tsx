...
const RoomManagement = () => {
  ...
  const fetchRooms = async (page = 1, limit = 10) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/rooms/getall?page=${page}&limit=${limit}`);
      const { rooms: RoomData = [], totalPages = 1 } = response.data;      // Filter based on role
      ...
      setRoomsTotalPages(totalPages || 1); // from backend response
      setRoomPages(page);
              ...
              <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto  max-h-[80vh] scrollbar-hide">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        ...
                        const selectedRate = selectedProperty?.rateCard?.find((r) => r.roomType === value);
                        setFormData({
                          ...formData, roomType: value,
                          monthlyRent: selectedRate ? selectedRate.rent : 0, // auto-fill rent
                          depositAmount: selectedRate ? selectedRate.rent * 2 : 0 //auto calculate and fill 
                        });
                      }} >
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        {formData.propertyId
                          ? properties.find((p) => p.id === formData.propertyId)?.rateCard.map((rc) => (
                            <SelectItem key={rc.id} value={rc.roomType}>
                              {rc.roomType} – ₹{rc.rent}
                            </SelectItem>
                          ))
                          : <SelectItem value="no-property" disabled>Select property first</SelectItem>}
                      </SelectContent>
                    </Select>
                  ...
                  <div>
                    ...
                    <Input type="number" readOnly value={formData.monthlyRent ?? ''} onChange={(e) => handleInputChange('monthlyRent', e.target.value ? Number(e.target.value) : null)} />
                    {errors.monthlyRent && <p className="text-red-500 text-sm">{errors.monthlyRent}</p>}
                  </div>
                  <div>
                    <Label>Deposit Amount (₹)</Label>
                    <Input type="number" readOnly value={formData.depositAmount ?? ''} onChange={(e) => handleInputChange('depositAmount', e.target.value ? Number(e.target.value) : null)} />
            ...
            {hasPermission('Room Management', 'read') ? (
              <>
                ...
                <div className="flex justify-end items-center mt-4 gap-2">
                  <Button
                    size="sm"
                    disabled={roomPages === 1}
                    onClick={() => fetchRooms(roomPages - 1)}
                  >
                    Prev
                  </Button>

                  <div>
                    Page {roomPages} of {roomsTotalPages}
                  </div>

                  <Button
                    size="sm"
                    disabled={roomPages === roomsTotalPages}
                    onClick={() => fetchRooms(roomPages + 1)}
                  >
                    Next
                  </Button>
